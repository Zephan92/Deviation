...
namespace Deviation.Data.LootPool
{
	public class LootPool : ILootPool
	{
		...
		public Dictionary<IResource, int> GetPool()
		{
			return _pool;
		}
		...
		private IResource GetRandomResource()
		{
			...
			int ranInt = _random.Next(0, _lootPoolLeft);
		...
		private Dictionary<IResource, int> ChooseResourceTypesInPool(int[] resourceTypeCountByRarity, int[] resourceCountByRarity)
		{
			...
			List<IResource>[] resourceListsByRarity = CreateResourceListsByRarity(new List<IResource>(ResourceLibrary.ResourceLibraryTable.Values), (int)Rarity.Count);
