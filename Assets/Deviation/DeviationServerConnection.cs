...
namespace Assets.Deviation
{
	...
	/// </summary>
	public class DeviationServerConnection : MonoBehaviour
	{
		...
		public BmLogger Logger = Msf.Create.Logger(typeof(DeviationServerConnection).Name);

		private static DeviationServerConnection _instance;
		...
		private IEnumerator StartConnection()
		{
			...
			while (true)
			{
				...
				if (!connection.IsConnected)
				{
					...
					_popupController.ToggleNotConnectedToServerPopup(true);
				}
		...
		{
			_popupController.ToggleNotConnectedToServerPopup(false);
			TimeToConnect = MinTimeToConnect;
